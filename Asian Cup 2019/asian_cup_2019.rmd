...
pacman::p_load(tidyverse, scales, lubridate, ggrepel, stringi,
               glue, extrafont, rvest, ggtextures, cowplot)
...
acup_winners_clean <- acup_winners_raw %>% 
  ...
  mutate(key = case_when(
           ...
           key == "third_num" ~ "3rd Place"
         ),
         key = key %>% fct_relevel(c("Champions", "Runners-up", "3rd Place"))) %>% 
  # hack-ish solution?
  arrange(key, value) %>% 
...
a <- acup_winners_clean %>% 
  ...
  labs(x = "Number of Occurrence",
       ...
       subtitle = "Four-time Champions, Japan, only won their first in 1992!",
       caption = glue("Note: Israel was expelled by the AFC in 1974 while Australia joined the AFC in 2006.")) +
  facet_wrap(~key) +
  theme_minimal() +
...
acup_kickoff %>% 
  ...
  mutate(time2 = dmy_hm(time),
         time_jp = with_tz(time2, tz = "Asia/Tokyo"),
         time_ac = with_tz(time2, tz = "Asia/Dubai"),
         time_ac = tz("Asia/Dubai"))
```
