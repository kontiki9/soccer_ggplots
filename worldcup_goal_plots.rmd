...
ani <- ggplot(pass_data) +
  ...
  coord_flip(xlim = c(49, 101),
             ylim = c(-1, 101)) +
  ggimage::geom_emoji(aes(x = x, y = y, frame = time), image = "26bd", size = 0.035) +
  geom_segment(data = goal_data, 
               aes(x = x, y = y, xend = x2, yend = y2), 
               linetype = "dashed", color = "red", size = 0.85) +
  geom_segment(data = golovin_data,
...
gazinsky_goal <- pass_tween %>% 
  ...
  ggimage::geom_emoji(
    ...
    image = "26bd", size = 0.035) +
  geom_label(
    aes(x = 94, y = 60, label = "Gazinsky"), hjust = -0.1) +
  geom_label(
    aes(x = 83, y = 23, label = "Golovin"), hjust = -0.05) +
  geom_label(
    aes(x = 75, y = 11, label = "Golovin"), hjust = -0.1) +
  geom_label(
    aes(x = 98, y = 0, label = "Zhirkov"), vjust = -0.3) +
  geom_label(
    aes(x = 84, y = 6, label = "Zhirkov"), vjust = -0.3) +
  geom_segment(data = goal_data, 
               aes(x = x, y = y, xend = x2, yend = y2), 
               linetype = "dashed", color = "red", size = 0.85) +
  geom_segment(data = golovin_data,
...
gganimate(gazinsky_goal, 
          ...
          title_frame = FALSE, interval = 0.05, 
          "gazinsky_goal.gif")

```


- reorder geom_emoji to bottom so soccer ball emoji can appaer above labels...
