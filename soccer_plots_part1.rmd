...
The important package here is the `ggsoccer` package made by Ben Torvey, check out the GitHub repo [here]() and 
...
You need to know the two-digit ISO codes so that the function nows which flags to look up
...
`geom_flag()` function from the `ggimage` package

(other options include packages like `ggflags`)

there is a better way to do this which I will show later!

- use soccer ball emoji instead of image

We can use the `emoji_search()` function from the emoGG package to find the emoji we want to use. Then use either emoGG or ggimage's `geom_emoji()` function to insert it into your ggplot!
...
```{r Osako winner plot, fig.width=6, fig.height=8}
...
ggplot(osako_gol) +
  ...
  annotate(geom = "text", family = "Dusha V5", 
           ...
           y = c(30, 30, 45, 81, 85)) +
  ggimage::geom_flag(aes(x = x, y = y,
                         image = image),       # Japan Flag
                     size = c(0.08, 0.08)) +
...
heavy usage of the `annotate()` function to get the labels exactly as we want them! Like before, we also had to expand the x-axis limits in the `coord_flip()`. This is also so we can insert the World Cup image on the top right without using grobs/Magick and such.

Grabbing World Cup logo: 

use ggimages and do the same thing as we did when we added the soccer ball image into the plot

as before we can place inside plot with `geom_image()` function and 


finding the ISO codes without having to look online!
...
pretty intuitive for countries like Japan and Colombia but when you're dealing with lots of countries like at the world cup or the olympics, having a reproducible workflow for this is very helpful!

the rest is the same as adding in the flags as shown previously with `geom_flag()`!
