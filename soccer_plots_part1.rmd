...
Putting that aside, let's plot out the 
To save space I do not show the data frames with the coordinate points, however you can find them [here]() on my GitHub!

### Gazinsky Scores The First Goal!

```{r gazinsky data, echo=FALSE}
library(ggplot2)
library(dplyr)
library(ggsoccer)
library(ggimage)
...
I was up quite late at night after seeing Portugal vs. Spain, a candidate for match of the tournament for the group stages, if not for the whole tournament!
In my excitement I drew up Cristiano Ronaldo's hattrick:

```{r Cristiano data, echo=FALSE}
library(ggplot2)
library(ggsoccer)
library(extrafont)
library(emoGG)
...
Compared to the first plot, we increased the x-limit so that we could place our `geom_text()` annotations and flag images together without having to use grobs. This also meant that we put the plot title and subtitle in the `geom_text()` rather than in the `labs()` function, which let all the text/label data be organized in one dataframe. 

To show the flags we use the `geom_flag()` function from the ggimage package. The function requires you to pass a two-digit ISO code in the `image` argument for the flags of the countries you want. You can find the ISO codes for countries with a quick google search, Portugal is "PT" and Spain is "ES".
...
Now, instead of the soccer ball image, we can use the `emoji_search()` function from the `emoGG` package to find the soccer ball emoji. Then use either emoGG or ggimage's `geom_emoji()` function to insert it into your ggplot!
...
Instead of the soccer ball image in the first graphic, I will be using the emoji version from now on!
...
It's a slightly hacky solution to include a lot of blank spaces between the country name and the score to put the flags in but I don't know of any geoms that can incorporate both text and images at the same time so the hacky solution will do!
...
```{r Osako winner data, echo=FALSE}
...
```{r Osako winner plot, fig.width=7, fig.height=5}
...
I could have used the `annotate()` function to add the little comment about Ospina being stuck in no-man's-land but I prefer to have all of my text in a single dataframe. Like before, we also had to expand the x-axis limits in the `coord_flip()`. This is also so we can insert the World Cup image on the top right without using grobs/Magick and such. To grab that World Cup logo, we do the same things as we did when we added the soccer ball image in the first plot with `ggimage`.

For finding the ISO codes to input for the `geom_flag()` function we can do one better than previous attempts by using the `countrycode` package to find ISO codes without having to look online!
...
Although the ISO codes are pretty intuitive for countries like Japan and Colombia but when you're dealing with lots of countries like at the world cup or the olympics, having a reproducible workflow for this is very helpful! 

These are goals just from the first week of the World Cup and if I had the time I would do more as there have been some fantastic individual and team goals so far. I will be making a few more and will post it on Twitter so keep an eye out there!

In a future part (as in not necessarily the next part), I want to animate some of these goal graphics. I've been slowly working my way through them in the past week so here is a preview:

- Gazinsky's goal is harder because there are lots of moving parts that absolutely need to be included.
for now i only have this one without tweenr ... i still need to work on a few things as i'm finding it tricky with multiple moving parts

still a work-in-progress, I am trying to figure out how to get the movement lines (the green ones) to disappear after the player has finished transitioning...

```{r gazinsky animate echo=FALSE}
library(ggplot2)
library(dplyr)
library(ggsoccer)
library(ggimage)
...
For Osako's goal, the tweenr version is basically done. Much easier as I simplified it so that the only moving bit to interpolate was the moving ball.

```{r echo=FALSE}

```

In the next week or so I'll have another part ready to showcase the fully tweenr-animated goals!
