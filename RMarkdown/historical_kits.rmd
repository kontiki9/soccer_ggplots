...
japan_kits <- list.files(path = "../japan_kits/1998/", pattern = "*.gif", full.names = TRUE) %>% 
  map(image_read) %>% 
  image_join()

# combine together side-by-side

japan_kits %>% image_append() # all 4 kits in one img

kit_labels <- data_frame(
  label = c("Japan (1) vs. (0) Argentina", "Japan (0) vs. (1) Croatia", "Japan (1) vs. (2) Jamaica")
)

japan_kits[c(4, 1:3)] %>% image_append()

japan_kits %>% 
  map2(4, 1:3, ~ image_append)

japan_kits[c(4, 3)] %>% 
  image_append() %>% 
  image_border(geometry = "10x40", color = "white") %>% 
  image_annotate(text = kit_labels$label, 
                 gravity = "south", location = "+0+300",
                 font = "Dusha V5", size = 30)
...
j_arg <- japan_kits[c(4, 1)] %>% image_append()  # vs. Argentina
j_cro <- japan_kits[c(4, 2)] %>% image_append()  # vs. Croatia
j_jam <- japan_kits[c(4, 3)] %>% image_append()  # vs. Jamaica

j_kits <- c(j_arg, j_cro, j_jam)
# make into function?

kit_list <-list(img = j_kits)

#kit_list %>% set_names(nm = c("img", "img", "img"))
...
for (i in seq_along(kit_list$img)) {
  
  kit_list$img[i] <- kit_list$img[i] %>% 
    image_border(geometry = "10x40", color = "white") %>% 
    image_annotate(text = kit_labels$label[i], 
                   gravity = "south",
                   location = "+0+300",
                   font = "Dusha V5",
                   size = 25)
...
jkits_ani %>% 
  image_animate(fps = 10) %>% 
  image_write(path = "japan_versus_kit.gif")
...
kit_labels_2002 <- data_frame(
  label = c("Japan (2) vs. (2) Belgium", "Japan (1) vs. (0) Russia", "Japan (2) vs. (0) Tunisia")
)

# 2 is Japan Away, 3 is Japan Home
# 1 = belgium, 4 = russia, 5 = tunisia
...
j_kits_2002 <- c(j_bel, j_rus, j_tun)
# make into function?
...
kit_labels_2010 <- data_frame(
  label = c("Japan (1) vs. (0) Cameroon", "Japan (0) vs. (1) Netherlands", "Japan (3) vs. (1) Denmark")
)
...
j_kits_2010 <- c(j_cam, j_ned, j_den)
...
kit_labels_2018 <- data_frame(
  label = c("Japan (2) vs. (1) Colombia", "Japan (2) vs. (2) Senegal", "Japan (0) vs. (1) Poland")
)

# 2 is Japan Home
# 1 = col, 3 = pol, 4 = sen
j_col <- japan_kits_2018[c(2, 1)] %>% image_append()  # vs. Colombia
j_sen <- japan_kits_2018[c(2, 4)] %>% image_append()  # vs. Senegal
j_pol <- japan_kits_2018[c(2, 3)] %>% image_append()  # vs. Poland

j_kits_2018 <- c(j_col, j_sen, j_pol)
...
japan_ani_2018 <- image_morph(c(kit_list_2018$img[1], kit_list_2018$img[1]), frame = 10)

kits_morph_j0 <- 2:3 %>% 
  map(~ image_morph(c(kit_list_2018$img[.x-1], kit_list_2018$img[.x]), frames = 10)) %>% 
  image_join()
...
jkits_ani_2018 <- c(japan_ani_2018, kits_morph_j0)
#jkits_ani <- c(jkits_ani, kits_morph_j1)  #  need to integrate PAUSE with morph anims
...
jkits_ani_ALL <- c(jkits_ani_1998, jkits_ani_2002, jkits_ani_2006, jkits_ani_2010, jkits_ani_2014, jkits_ani_2018)

jkits_ani_ALL %>% 
  image_animate(fps = 10) %>% 
  image_write(path = "japan_versus_ALL.gif")
