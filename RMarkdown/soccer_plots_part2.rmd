...
Capturing World Cup drama: Visualizing changes in the group standings during the final matchday

This is __Part 2__ of my World Cup Data Viz Series (See Part 1 [here](https://ryo-n7.github.io/2018-06-29-visualize-worldcup/))! I was originally planning for my animated goals to be Part 2 but that will have to wait for now. In this blog post I'll be showing you the visualizations I made for how the group standings changed throughout the final matchday that I've been posting on [Twitter](https://twitter.com/R_by_Ry)o. 

I thought I'll go through the entire code give me something to refer to for future visualizations, it might be a good sounding-board for improving it, and eventually creating some kind of function/package for this set of graphs!

Besides the code I will go through some of the design/ggplot2 choices I made as well.

Here's an example using __Group H__ (totally not biased or anything...):

[]() >>> make not on tie-breakers as caption??
...
The most important part of the plot is the dataframe that houses each team's ranking during the final 90 minutes, there is a new column in the graph each time the ranking changes due to a goal or yellow/red card. You can take a look at the tie-breakers [here]().
...
As previously mentioned in [Part 1]() you can use the `countrycode()` function from the `countrycode` package to easily extract the __ISO__ codes needed for the `geom_flag()` function when it comes to plotting.
...
I made things slightly easier on myself by using the `rep()` function a lot. This is done so that it guards against any typos I make when creating the individual dataframes. Now, I only have to make changes to the initial dataframe's `positions` column and the other dataframes will expand/contract as needed. When this happens I will of course need to add or subtract a label as necessary but this saves me time from individually deleting a series of `x/y` coordinate numbers. I can only do this for columns which have a series of `positions` that are strictly set, like in the `country_labs` dataframe there are only ever going to be `1, 2, 3, 4` y-axis ticks. For the `points_labs` dataframe, I use `rep()` with `max()` as the points labels are only ever going to be found in the last `spread()`ed `time` column, or in other words, the largest number in the `time` row in the initial dataframe.
...
One thing that I always had to keep in mind while creating the initial "base" dataframe was that not __ALL__ the goals scored caused a change in the group standings. This meant that I needed to be constantly thinking backwards as to how the group table changed before/after the goal was scored. In addition, it was difficult to keep in mind the different [tie-breakers]() (goal difference, head-to-head score, yellow/red cards, etc.) involved between the teams at every different event point in the games. Thankfully (though not for a match quality point of view) some group rankings were already completely decided by Matchday 2 and required very little cognitive effort on my part, like Groups __A__, __C__, and __E__.

The __key__ point of these graphs were to emphasize the _movement_ of teams in the group table as a result of certain events. Therefore, I had to also be wary of not introducing too much detail into these plots so I can direct the focus on the flags rather than the labels. 

Along that line of reasoning, I at first was thinking about having the goal scorer text be inside `geom_label()` instead of `geom_text()` but I ultimately didn't as I thought it would obscure the dotted movement lines too much, especially since some of the player names and the "pens." or "o.g." took up a lot of space.

I also originally had the total group points in a darker black but I figured it made the graph look lopsided. I changed all the surrounding axis text to be "grey30" so that the axis labels would "surround" the plot and direct focus towards the flags inside. 

A possible point of confusion could be that it can be hard to tell which teams are playing which in the final matchday, without a clear label it can be confusing as a goal in one game may not affect the teams in that match but can completely change the rankings of the teams in the other game! In the end, the best I could do was to include the matchups in the __subtitle__ of the plot but I'm still thinking about whether there's a more effective method.

The most difficult part of this whole process of making a graph for each group was the problem of iteration. Each group had different amounts of goals or rank-changing events as well as more aesthetic things such as the length of the country names and goal scorer names that made tweaking each plot individually a necessity. It would have saved me a lot of time if I could have turned this into a package somehow but with all the moving pieces of this puzzle and how they are interconnected with eachother I wasn't able to. These were still fun to make and I plan to make some more for next season's Champions League groups as well as for the Asian Cup in January of 2019 and maybe even for the Asian Games next month so creating a package may be something I turn my full attention to once this World Cup is over! On a positive note, I'm very thankful that there were no blowouts on the final matchday as fitting in the goal scorer labels would've been an absolute nightmare!

----------------------------------------------------------------------------------

- take another look at plots >>> fix the margins for ALL OF THEM.
