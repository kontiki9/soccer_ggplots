...
```{r tweenr offside, fig.height=6, fig.width=4}
offside_list <- map(seq(nrow(trap_data)),
                      ~ trap_data[c(seq(.x), rep(.x, nrow(trap_data) - .x)), ])
  
offside_tween <- offside_list %>% 
  tween_states(tweenlength = 1.5, statelength = 0.0001, ease = "linear", nframes = 50)
