...
point_data <- data.frame(x = c( 100, 83, 100, 83, 88.5, 100, 83, 100, 100),
                         y = c(   0, 21,  21, 38, 50,    50, 80,  80, 100),
                         label = c("100, 0", "83, 21", "100, 21", "83, 38", "88.5, 50", "100, 50", "83, 80", "100, 80", "100, 100"))
...
ball_list <- map(seq(nrow(ball_data)),
                      ~ ball_data3[c(seq(.x), rep(.x, nrow(ball_data) - .x)), ])
  
ball_tween <- ball_list %>% 
  tween_states(tweenlength = 0.5, statelength = 0.00000001, ease = "linear", nframes = 75)

data_t <- ball_tween %>% group_by(.frame) %>% slice(9)
...
```{r tweenr offside, fig.height=10, fig.width=8}
flag_data <- data.frame(
  x = c( 42, 72),
  y = c(107, 107),
  team = c("japan", "senegal")
...
offside_list <- map(seq(nrow(trap_data)),
                      ~ trap_data[c(seq(.x), rep(.x, nrow(trap_data) - .x)), ])
  
offside_tween <- offside_list %>% 
  tween_states(tweenlength = 0.5, statelength = 0.00000001, ease = "linear", nframes = 50)

off_t <- offside_tween %>% group_by(.frame) %>% slice(5)
